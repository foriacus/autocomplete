this.AutoComplete = (function() {    
    ...
    var template = '<div class="{{classPrefix}}"><ul class="{{classPrefix}}-ctn" data-role="items">{{#each items}}'+
                '<li data-role="item" class="{{../classPrefix}}-item" data-label="{{label}}" data-value="{{value}}">{{highlightItem label value ../classPrefix}}</li>' +
        '{{/each}}</ul></div>';
    ...
    var AutoComplete = Overlay.extend({
        ...
        selectItem: function() {
            ...
            if (item) {
                ...
                var matchLabel = item.attr('data-label');
                this.get('trigger').val(matchLabel);
                this.get("trigger").attr("pinyin",matchKey);
                this.set('inputValue', matchLabel);
                this._clear();
            }
        },
    ...
    function highlightItem(label, value, classPrefix) {
        var index = this.highlightIndex,
            cursor = 0, v = value || this.value || '', h = '';
        if ($.isArray(index)) {
            ...
            return new Handlebars.SafeString(label + '(' + h + ')');
        }
        return label + v;
    }
    return AutoComplete;
